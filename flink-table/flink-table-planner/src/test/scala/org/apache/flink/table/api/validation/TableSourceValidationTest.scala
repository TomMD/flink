...
import org.apache.flink.table.utils.{TestBoundedTableSource, TestTableSourceWithTime}
...
class TableSourceValidationTest {
  ...
  def testBoundedTableSourceForStreamEnv(): Unit = {
    ...
    val ts = new TestBoundedTableSource(schema, rowType, Seq[Row]())

    // should fail because BoundedTableSource is not supported in StreamTableEnvironment
    tEnv.registerTableSource("testTable", ts)
  }
}
