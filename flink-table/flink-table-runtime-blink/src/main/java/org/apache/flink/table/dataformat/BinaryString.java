...
public final class BinaryString extends LazyBinaryFormat<String> implements Comparable<BinaryString> {
	...
	/**
	 * Creates an BinaryString from given UTF-8 bytes with offset and numBytes.
	 */
	...
	/**
	 * Returns a substring of this. The substring begins with the code point at the specified
	 * index and extends to the end of this BinaryString.
	 *
	...
	public BinaryString substring(final int start, final int until) {
		...
		if (inFirstSegment()) {
			...
			return substringMultiSegs(start, until);
		}
	}
	...
	BinaryString copyBinaryStringInOneSeg(int start, int end) {
		int len = end - start + 1;
