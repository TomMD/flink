...
public class FlinkKinesisConsumerTest {
	...
	public void testSourceSynchronization() throws Exception {
		...
		assertEquals(0, TestWatermarkTracker.WATERMARKS.size());

		// trigger sync
		testHarness.setProcessingTime(testHarness.getProcessingTime() + 1);
		assertTrue("global watermark", TestWatermarkTracker.WATERMARKS.size() == 1);
	...
	private static class TestWatermarkTracker extends WatermarkTracker {
		...
		private static final Map<String, WatermarkState> WATERMARKS = new ConcurrentHashMap<>();

		@Override
		protected void refreshWatermarkSnapshot(Map<String, WatermarkState> watermarks) {
			watermarks.putAll(WATERMARKS);
		}

		@Override
		protected void saveWatermark(String id, WatermarkState ws) {
			WATERMARKS.put(id, ws);
		}

		static void assertSingleWatermark(long expected) {
			Assert.assertEquals(1, WATERMARKS.values().size());
			Assert.assertEquals(expected, WATERMARKS.values().iterator().next().getWatermark());
		}
	}
}
