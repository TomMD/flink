...
public class InputOutputFormatVertex extends JobVertex {
	...
	public void initializeOnMaster(ClassLoader loader) throws Exception {
		final InputOutputFormatStub formatStub = initInputOutputFormatStub(loader);
		...
		try {
			...
			Map<OperatorID, UserCodeWrapper<? extends InputFormat<?, ?>>> inputFormats = formatStub.getInputFormats();
			...
			for (Map.Entry<OperatorID, UserCodeWrapper<? extends InputFormat<?, ?>>> entry : inputFormats.entrySet()) {
				...
				try {
					...
					inputFormat.configure(formatStub.getParameters(entry.getKey()));
				} catch (Throwable t) {
			...
			Map<OperatorID, UserCodeWrapper<? extends OutputFormat<?>>> outputFormats = formatStub.getOutputFormats();
			for (Map.Entry<OperatorID, UserCodeWrapper<? extends OutputFormat<?>>> entry : outputFormats.entrySet()) {
				...
				try {
					...
					outputFormat.configure(formatStub.getParameters(entry.getKey()));
				} catch (Throwable t) {
	...
	private InputOutputFormatStub initInputOutputFormatStub(ClassLoader classLoader) throws Exception {
		try {
			return new InputOutputFormatStub(new TaskConfig(getConfiguration()), classLoader);
		} catch (Throwable t) {
