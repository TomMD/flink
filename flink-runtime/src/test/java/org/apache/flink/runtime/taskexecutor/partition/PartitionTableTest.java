...
public class PartitionTableTest extends TestLogger {
	...
	public void testEmptyTable() {
		...
		Collection<ResultPartitionID> partitionsForNonExistingJob = table.stopTrackingPartitions(new JobID());
		assertNotNull(partitionsForNonExistingJob);
		assertEquals(0, partitionsForNonExistingJob.size());

		assertFalse(table.hasTrackedPartitions(new JobID()));
	}
	...
	public void testStartTrackingPartition() {
		...
		table.startTrackingPartition(JOB_ID, PARTITION_ID);
	...
	@Test
	public void testStopTrackingSinglePartition() {
		final ResultPartitionID partitionId2 = new ResultPartitionID();
		final PartitionTable table = new PartitionTable();

		table.startTrackingPartition(JOB_ID, PARTITION_ID);
		table.startTrackingPartition(JOB_ID, partitionId2);

		table.stopTrackingPartition(JOB_ID, partitionId2);
		assertTrue(table.hasTrackedPartitions(JOB_ID));
	...
	@Test
	public void testStopTrackingSpecificPartitions() {
		final ResultPartitionID partitionId2 = new ResultPartitionID();
		final PartitionTable table = new PartitionTable();
